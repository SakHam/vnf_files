...
def start():
  ...
  if camera_back_up == "NONE" :
    ...
    #cmd = 'sudo docker run -d --name ' + camera_name + ' --rm --privileged --shm-size=1g -v /dev/bus/usb:/dev/bus/usb -v ' + path  + ':/frigate_' + camera_name + ':ro -p ' + docker_ports + ' -e RTSP_PASSWORD="admin" frigate2:latest'   
    os.system(cmd)
